...
namespace Semanticus.Engine
{
    ...
    {
        [McpServerTool(Name = "list_evidence"), Description("EVIDENCE: list the sealed Test and Workflow evidence saved with the open model under `.semanticus/evidence/<model-identity>`. Each item includes type, producer, verdict with coverage, content signature, paths, and verification state. A changed or conflicted record remains listed as invalid and is never trusted or hidden. Saved reports is a Semanticus Pro feature. Read-only. Next: get_evidence(id) to review one; save_evidence(source) to explicitly retain the latest Test report or a terminal Workflow run.")]
        public static Task<EvidenceLibrary> ListEvidence(IEngine engine) => engine.ListEvidenceAsync();

        [McpServerTool(Name = "get_evidence"), Description("EVIDENCE: open one current-model record from list_evidence as canonical JSON plus its deterministic self-contained HTML view. The engine re-verifies the signature and HTML pairing before returning it; invalid evidence is refused with the exact reason. Saved reports is a Semanticus Pro feature. Read-only.")]
