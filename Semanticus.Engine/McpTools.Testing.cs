...
namespace Semanticus.Engine
{
    ...
    public static class McpToolsTesting
    {
        ...
        public static Task<TestSuiteRunResult> RunTests(IEngine engine,
            [Description("Also append this run's health to .semanticus/tests/runs.jsonl: the drift-trend history (Pro; free runs are evaluated in full but not stored). A partial scope cannot be recorded.")] bool persist = false,
            ...
            [Description("Optional section names to run: measures, relationships, security, interview. Omit for every section. A partial list never mints a grade and cannot be recorded.")] string[] sections = null)
            => engine.RunTestSuiteAsync(persist, "agent", only, sections);

        [McpServerTool(Name = "try_test"), Description("TESTS: run one unsaved test definition once and return its outcome. Writes nothing to the suite. kind='measureValue' checks a measure against a trusted number; kind='measureReconcile' is the SQL-vs-DAX reconciliation. Offline the outcome is NotVerifiable. Trying is FREE; saving the definition is Pro (save_test).")]
