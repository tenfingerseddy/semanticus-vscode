...
export const MODEL_SCOPED_KINDS = new Set(['table', 'function', 'calcgroup', 'role', 'perspective', 'model']);

/** 'column:Sales/Amount' -> { kind:'column', table:'Sales', name:'Amount' }. Standalone copy (no vscode import). */
export function refPartsOf(ref: string): { kind: string; table?: string; name: string } {
    const colon = ref.indexOf(':');
    const kind = colon >= 0 ? ref.slice(0, colon) : '';
    const rest = colon >= 0 ? ref.slice(colon + 1) : ref;
    if (MODEL_SCOPED_KINDS.has(kind)) return { kind, name: rest };   // whole remainder is the name (may contain '/')
...
function br(s: string): string { return '[' + s.replace(/]/g, ']]') + ']'; }
function unbr(s: string): string { return s.replace(/]]/g, ']'); }
 ...
 */
export function buildDaxHeader(ref: string): string | null {
...
export function headerKeywordFor(kind: string): string | null {
    switch (kind) {
 ...
 */
export function parseDaxHeader(line: string): { keyword: string; table?: string; name: string } | null {
    const t = line.trim();
    ...
    const mid = m[2];
    switch (keyword) {
        case 'MEASURE': case 'COLUMN': case 'CALCULATIONITEM': {
 ...
 */
export function parseDaxHeaderName(line: string): string | null {
    return parseDaxHeader(line)?.name ?? null;
}

export type DaxHeaderCheck = { ok: true; name: string } | { ok: false; reason: string };
 ...
 */
export function checkDaxHeader(ref: string, line: string): DaxHeaderCheck {
...
/**
 * The header-doc identity CARRIED BY THE URI (CRITICAL 2). Content is not a safe identity -- after a model swap the
 * in-memory registry clears, so a dirty doc from model A whose line 1 still validates could be re-adopted and rename /
 * write model B's same-named object. The virtual FS mints the uris, so it stamps `isHeaderUri` + the OWNING model's
 * key onto them at create time. Uris persist across window reloads (editor layout restore), so this survives a reload
 * where the registry would be gone, and the embedded model key makes cross-model reuse detectable.
 *   isHeaderUri   = the uri is a header doc (carries the create-then-edit name header on line 1)
 *   uriModelKey   = the model the header doc was minted for (a stable per-model token; see identityToken)
 *   modelKey      = the CURRENTLY open model's key (fetched authoritatively at Save, not a cached global)
 * The two keys are optional at the type level (a uri can arrive without them), but decideDaxSave treats a MISSING key
 * on either side as corrupt and rejects -- a header uri must always carry identity (CRITICAL 1).
 */
export interface DaxHeaderIdentity { isHeaderUri: boolean; uriModelKey?: string; modelKey?: string }

export type ModelMatch = { ok: true } | { ok: false; reason: string };
 ...
 */
export function guardModelMatch(uriModelKey: string | undefined, liveModelKey: string | undefined): ModelMatch {
...
export function decideDaxSave(ref: string, header: string, body: string, id: DaxHeaderIdentity): DaxSaveDecision {
    ...
    if (!match.ok) return { kind: 'reject', reason: match.reason };
    const v = checkDaxHeader(ref, header);
    if (!v.ok) return { kind: 'reject', reason: v.reason };
...
/**
 * A persisted partial-rename record: the object's committed new ref + the session it was renamed in (model token +
 * session id) + the session revision the rename's result reported. `state: 'dead'` marks a TOMBSTONE (CRITICAL 2): a
 * record whose rename can never again be safely resumed is NOT pruned — pruning would drop the pending guard and let the
 * still-dirty editor later save against an UNRELATED object recreated at the new name (renaming it and overwriting it
 * with the preserved DAX). A tombstone instead makes every future save from this editor a permanent, fail-closed refusal
 * until the doc is closed clean or swept away.
 *
 * `sessionId` + `revision` (round 8) are LOAD-BEARING, not just provenance: refs are NAME-based, so once the model has
 * mutated at all since the rename committed, an object living at the new name is NOT provably the one we renamed (a
 * delete+recreate could have put an unrelated impostor there, and no liveness probe can tell them apart). So the ONLY
 * safe resume is the IMMEDIATE retry — the SAME session with the session revision STILL EXACTLY the rename's — and the
 * write is CAS-fenced on that revision. Any drift (a different session, or the revision moved) refuses permanently. See
 * decideRenameRecovery.
 */
export interface PendingRenameRecord { newRef: string; modelKey: string; sessionId?: string; revision?: number; state?: 'dead' }

export type RenameRecovery =
    | { kind: 'proceed'; baseRef: string }              // treat baseRef as the object's real current ref for the rename-then-body flow
    | { kind: 'reject'; reason: string; tombstone?: boolean };  // tombstone = persist/keep this record as a fail-closed 'dead' marker (NEVER delete it — a still-dirty doc could otherwise save against a recreated same-name object)
 ...
 */
export function decideRenameRecovery(
    record: PendingRenameRecord | undefined,
    oldRef: string,
    liveModelKey: string | undefined,
    liveSessionId: string | undefined,
    liveRevision: number | undefined,
): RenameRecovery {
    if (!record) return { kind: 'proceed', baseRef: oldRef };
...
export function identityToken(key: string): string {
    const enc = Buffer.from(key, 'utf8').toString('base64url');   // reversible, url-safe, decode-invariant
    if (enc.length <= 512) return enc;                            // carry the exact identity: no collision possible
...
export function splitDaxHeader(text: string): { header: string; body: string; headerLen: number } {
    ...
    if (nl < 0) return { header: text, body: '', headerLen: text.length };
 ...
 */
export function reKeyRef(ref: string, oldName: string, newName: string): string {
...
export function uniqueName(base: string, existingLower: Set<string>, sep = ' '): string {
