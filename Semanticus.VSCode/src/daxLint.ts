...
export interface DaxIndex {
  tables: Set<string>;                        // lowercased table names
  columnsByTable: Map<string, Set<string>>;   // lower table -> set of lower column names
  measures: Set<string>;                      // lowercased measure names
  allColumns: Set<string>;                    // lowercased column names across all tables
}

export type RawDiag = { start: number; end: number; message: string; severity: 'error' | 'warning' };
...
export function maskDax(text: string, maskIdents: boolean): string {
  const a = text.split('');
  const n = a.length;
  const blank = (s: number, e: number) => { for (let k = s; k < e && k < n; k++) if (a[k] !== '\n' && a[k] !== '\r') a[k] = ' '; };
  let i = 0;
  while (i < n) {
    const c = text[i];
    if (c === '"') {
      let j = i + 1;
      while (j < n) { if (text[j] === '"') { if (text[j + 1] === '"') { j += 2; continue; } break; } j++; }
      blank(i, j + 1); i = j + 1; continue;
    }
    if (c === "'") {
      let j = i + 1;
      while (j < n) { if (text[j] === "'") { if (text[j + 1] === "'") { j += 2; continue; } break; } j++; }
      if (maskIdents) blank(i, j + 1);
      i = j + 1; continue;
    }
    if (c === '/' && text[i + 1] === '/') { let j = i + 2; while (j < n && text[j] !== '\n') j++; blank(i, j); i = j; continue; }
    if (c === '-' && text[i + 1] === '-') { let j = i + 2; while (j < n && text[j] !== '\n') j++; blank(i, j); i = j; continue; }
    if (c === '/' && text[i + 1] === '*') { let j = i + 2; while (j < n && !(text[j] === '*' && text[j + 1] === '/')) j++; blank(i, Math.min(j + 2, n)); i = j + 2; continue; }
    i++;
  }
  return a.join('');
}

function balance(masked: string): RawDiag[] {
  const out: RawDiag[] = [];
  const stack: { ch: string; pos: number }[] = [];
  const opener: Record<string, string> = { ')': '(', ']': '[' };
  for (let i = 0; i < masked.length; i++) {
    const c = masked[i];
    if (c === '(' || c === '[') stack.push({ ch: c, pos: i });
    else if (c === ')' || c === ']') {
      const top = stack.pop();
      if (!top || top.ch !== opener[c]) out.push({ start: i, end: i + 1, message: `Unmatched '${c}'`, severity: 'error' });
    }
  }
  for (const s of stack) out.push({ start: s.pos, end: s.pos + 1, message: `Unclosed '${s.ch}'`, severity: 'error' });
  return out;
}

function references(masked: string, idx: DaxIndex): RawDiag[] {
  const out: RawDiag[] = [];
  ...
  while ((m = reUnq.exec(masked))) {
    ...
    if (!idx.tables.has(tlow) || !m[2]) continue;
    const cols = idx.columnsByTable.get(tlow);
...
export interface DaxSymbol {
  name: string;
  kind: 'var' | 'return';
  nameStart: number; nameEnd: number;     // the identifier range (selection range)
  fullStart: number; fullEnd: number;     // the whole declaration / clause range
}
...
export function extractDaxSymbols(text: string): DaxSymbol[] {
  const masked = maskDax(text, false);
  const raw: { kind: 'var' | 'return'; name: string; kwStart: number; nameStart: number; nameEnd: number }[] = [];
...
export function varDefinition(text: string, name: string): string | null {
...
export function collectVars(text: string): string[] {
  const masked = maskDax(text, false);
...
export function analyzeDax(text: string, idx: DaxIndex | null): RawDiag[] {
  const masked = maskDax(text, true);
  const diags = balance(masked);
  diags.push(...arityCheck(masked));
  if (idx) diags.push(...references(maskDax(text, false), idx));
...
function arityCheck(masked: string): RawDiag[] {
  const out: RawDiag[] = [];
  ...
  while ((m = re.exec(masked))) {
    ...
    if (counted.end < 0) continue;
    const [min, max] = spec;
    if (counted.count >= min && counted.count <= max) continue;
...
function countArgs(s: string, openParen: number): { count: number; end: number } {
  ...
  for (let i = openParen; i < s.length; i++) {
    ...
    if (c === ')') {
      depth--;
      if (depth === 0) return { count: any ? count : 0, end: i };
      continue;
    }
    if (c === ',' && depth === 1) { count++; continue; }
    if (depth === 1 && !/\s/.test(c) && !any) { any = true; count = 1; }
  }
