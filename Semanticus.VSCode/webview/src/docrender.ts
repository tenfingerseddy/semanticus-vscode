...
// The exported FILE keeps its filter script; the in-app preview must not carry one. That iframe is sandboxed
// without allow-scripts (deliberately), so every open of Model > Docs logged two "Blocked script execution in
// about:srcdoc" console errors for a script that could never run (M21, walkthrough 2026-09-14). Built by
// removing the exact strings this file emitted, so there is no second render pass and nothing to drift.
export function renderDoc(dto: DocModelDto, config: DocConfig, branding: DocBranding): { html: string; markdown: string; previewHtml: string } {
  const html = renderHtml(dto, config, branding);
  const previewHtml = html.split(searchScript()).join('').split(SEARCH_ONINPUT).join('');
  return { html, previewHtml, markdown: renderMarkdown(dto, config) };
}
...
function renderHtml(dto: DocModelDto, cfg: DocConfig, b: DocBranding): string {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>`
    ...
    + `<input id="doc-search" class="doc-search" type="search" placeholder="Filter…"${SEARCH_ONINPUT}/>`
...
function narrativeHtml(n: DocNarrative): string {
  const who = n.author === 'agent' ? 'Your assistant' : n.author === 'human' ? 'You' : '';
...
// The handler attribute is a named constant because the preview strips it by exact match, not by pattern.
const SEARCH_ONINPUT = ' oninput="docFilter(this.value)"';

// A tiny inline filter script for the EXPORTED file the user opens. The in-app preview strips it (see renderDoc):
// its iframe has no allow-scripts, so leaving it in only produced console errors.
