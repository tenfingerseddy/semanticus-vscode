...
let propGrid; // module-scoped so the "Properties" menu can target a node
let daxFs; // module-scoped so connectEngine() can re-attach it on restart
...
let healthSessionId; // which session the chip reflects — a session swap hides it (no didChange fires on open)
...
let referenceTree; // the "Reference Model" tree — a second model browsed to copy FROM
let referenceView;
let referenceRef; // which model the Reference view is browsing
let copyStash; // Ctrl+C in the Reference tree → Ctrl+V into the Model tree
let treeCopyStash; // Ctrl+C in the MODEL tree → Ctrl+V duplicates via the engine (a reference, not content)
let lastCopyFrom; // when both clipboards hold something, the most recent copy wins (real clipboard semantics)
let extCtx; // module-scoped so secure storage and one-time migrations share the activation context
async function activate(context) {
    ...
    vscode.commands.registerCommand('semanticus.newTable', () => authorNewTable()), vscode.commands.registerCommand('semanticus.newCalcTable', () => authorNewCalcTable()), vscode.commands.registerCommand('semanticus.newCalcGroup', () => authorNewCalcGroup()), vscode.commands.registerCommand('semanticus.newRole', () => authorNewRole()), vscode.commands.registerCommand('semanticus.newFunction', () => authorNewFunction()), vscode.commands.registerCommand('semanticus.newMeasure', (n) => authorNewMeasure(n)), 
    ...
    vscode.commands.registerCommand('semanticus.scriptDax', (n, ns) => scriptObjectsCmd('dax', n, ns)), vscode.commands.registerCommand('semanticus.scriptTmdl', (n, ns) => scriptObjectsCmd('tmdl', n, ns)), vscode.commands.registerCommand('semanticus.scriptTmsl', (n, ns) => scriptObjectsCmd('tmsl', n, ns)), vscode.commands.registerCommand('semanticus.applyDaxScript', () => applyDaxScriptCmd()), vscode.commands.registerCommand('semanticus.applyTmdlScript', () => applyTmdlScriptCmd()), vscode.commands.registerCommand('semanticus.hideObject', (n, ns) => setHiddenCmd(true, n, ns)), vscode.commands.registerCommand('semanticus.showObject', (n, ns) => setHiddenCmd(false, n, ns)), vscode.commands.registerCommand('semanticus.deleteObject', (n, ns) => authorDelete(n, ns)), 
    // Measure-specific.
    vscode.commands.registerCommand('semanticus.setFormat', (n) => setFormatCmd(n)), vscode.commands.registerCommand('semanticus.timeIntelligence', (n) => timeIntelCmd(n)), 
    // Column-specific.
    vscode.commands.registerCommand('semanticus.summarizeBy', (n) => summarizeByCmd(n)), vscode.commands.registerCommand('semanticus.dataCategory', (n) => dataCategoryCmd(n)), 
    // Table-specific.
    vscode.commands.registerCommand('semanticus.markDateTable', (n) => markDateTableCmd(n)), vscode.commands.registerCommand('semanticus.previewTableData', (n) => previewTableDataCmd(context, n)), vscode.commands.registerCommand('semanticus.updateSchema', (n) => updateSchemaCmd(context, n)), vscode.commands.registerCommand('semanticus.addToDiagram', (n, ns) => addToDiagramCmd(context, n, ns)), 
    ...
    vscode.commands.registerCommand('semanticus.showLineage', (n) => { if (n?.ref)
        navigateStudio(context, 'lineage', n.ref); }), vscode.commands.registerCommand('semanticus.editMCode', (n) => {
        try {
            ...
            if (n?.ref && refParts(n.ref).kind === 'partition') {
                navigateStudio(context, 'mcode', n.ref);
                return;
            }
            const t = tableOfNode(n);
            if (t)
                navigateStudio(context, 'mcode', `table:${t}`);
            else
                vscode.window.showInformationMessage('Right-click a table or partition to edit its M code.');
        }
        catch (e) {
            vscode.window.showErrorMessage('Edit M Code failed: ' + (e?.message ?? e));
        } // never silent
    }), 
    ...
    vscode.commands.registerCommand('semanticus.studioNextTab', () => navigateStudio(context, 'cycle:next')), vscode.commands.registerCommand('semanticus.studioPrevTab', () => navigateStudio(context, 'cycle:prev')), vscode.commands.registerCommand('semanticus.studioGoGroup', (g) => navigateStudio(context, 'group:' + (g || 'understand'))), vscode.commands.registerCommand('semanticus.studioGoTab', (t) => { if (t)
        navigateStudio(context, t); }), // power users: bind your own key to any tab id
    vscode.commands.registerCommand('semanticus.scanReadiness', () => navigateStudio(context, 'readiness', 'rescan')), vscode.commands.registerCommand('semanticus.scanBpa', () => navigateStudio(context, 'bpa')), // the BPA tab scans on open
    vscode.commands.registerCommand('semanticus.keyboardShortcuts', () => navigateStudio(context, 'shortcuts')), // the '?' cheat sheet
    vscode.commands.registerCommand('semanticus.amFieldParameter', () => navigateStudio(context, 'advmodels', 'area:fieldparams')), vscode.commands.registerCommand('semanticus.amCalcGroup', () => navigateStudio(context, 'advmodels', 'area:calcgroups')), vscode.commands.registerCommand('semanticus.amCalendar', () => navigateStudio(context, 'advmodels', 'area:calendars')), vscode.commands.registerCommand('semanticus.amPerspectives', () => navigateStudio(context, 'advmodels', 'area:perspectives')), vscode.commands.registerCommand('semanticus.amRlsOls', () => navigateStudio(context, 'advmodels', 'area:rlsols')), vscode.commands.registerCommand('semanticus.amDaxLib', () => navigateStudio(context, 'advmodels', 'area:daxlib')), 
    // Ship-gater: wire the user's own Claude Code (a separate product that reads only its OWN .mcp.json) to this engine.
    vscode.commands.registerCommand('semanticus.connectClaudeCode', () => connectClaudeCodeCmd()), vscode.commands.registerCommand('semanticus.setReferenceModel', () => setReferenceModelCmd()), vscode.commands.registerCommand('semanticus.refreshReferenceModel', () => loadReferenceModel()), vscode.commands.registerCommand('semanticus.clearReferenceModel', () => clearReferenceModel()), vscode.commands.registerCommand('semanticus.copyRefToModel', (n, ns) => copyRefIntoModel(ns?.length ? ns : [n])), vscode.commands.registerCommand('semanticus.copyRefObject', () => copyRefStash()), vscode.commands.registerCommand('semanticus.pasteIntoModel', () => pasteIntoModelCmd()), 
    // Partition-specific.
    vscode.commands.registerCommand('semanticus.refreshPartition', (n) => refreshPartitionCmd(n)), 
    // Relationship-specific.
    vscode.commands.registerCommand('semanticus.relCrossFilter', (n) => relCrossFilterCmd(n)), vscode.commands.registerCommand('semanticus.relToggleActive', (n) => relToggleActiveCmd(n)), 
    // Role-specific.
...
let licenseConflictPromptedKey;
...
let licenseMutationChain = Promise.resolve();
function withLicenseMutation(work) {
...
async function getLicenseToken(context = extCtx, opts) {
    const reconcile = opts?.reconcile !== false;
    let secret = '';
    try {
        secret = (await context.secrets.get(LICENSE_SECRET_KEY))?.trim() || '';
    }
    catch { /* keychain unavailable — fall through to the legacy setting */ }
    const legacy = (vscode.workspace.getConfiguration('semanticus').get('licenseToken') || '').trim();
    if (secret) {
    ...
    if (reconcile) {
        ...
        const migrated = await withLicenseMutation(async () => {
            try {
                if (((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim())
                    return false;
            }
            catch {
                return false;
            }
            const legacyNow = (vscode.workspace.getConfiguration('semanticus').get('licenseToken') || '').trim();
            if (legacyNow !== legacy)
                return false;
            return storeLicenseSecretVerified(context, legacy);
        });
        if (migrated)
            void clearLegacyLicenseSetting(true, legacy);
    }
    return legacy;
}
...
function reconcileLicenseConflict(context, legacyAtPrompt, secretAtPrompt) {
    ...
    if (licenseConflictPromptedKey === promptKey)
        return; // this exact pair is already prompted (or answered)
    ...
    const releaseGuard = () => { if (licenseConflictPromptedKey === promptKey)
        licenseConflictPromptedKey = undefined; };
    void vscode.window.showWarningMessage('Semanticus found two different Pro license tokens: one in secure storage and one in settings.json (plaintext). They do not match, so one is out of date. Which should be used?', 'Use settings value', 'Keep stored license').then(async (pick) => {
        ...
        const outcome = await withLicenseMutation(async () => {
            ...
            if (licenseMutationRevision !== revisionAtPrompt)
                return 'stale-committed';
            if (licenseMutationAttemptEpoch !== epochAtPrompt)
                return 'stale-attempted';
            ...
            let secretNow = '';
            try {
                secretNow = ((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim();
            }
            catch {
                return 'stale-unverified';
            }
            const legacyNow = (vscode.workspace.getConfiguration('semanticus').get('licenseToken') || '').trim();
            if (secretNow !== secretAtPrompt || legacyNow !== legacyAtPrompt)
                return 'stale-committed';
            if (pick === 'Use settings value' && !(await storeLicenseSecretVerified(context, legacyAtPrompt))) {
                ...
                try {
                    ...
                    return 'store-failed-unreadable';
                }
            }
        ...
        if (outcome === 'stale-committed' || outcome === 'stale-attempted' || outcome === 'stale-unverified') {
            ...
            releaseGuard();
            vscode.window.showInformationMessage(outcome === 'stale-committed'
            ...
            vscode.window.showInformationMessage('Semanticus: the settings.json token is now stored in secure storage.' +
...
async function storeLicenseSecretVerified(context, token) {
...
function expectedFullyCleared(res) {
...
function definedLegacyScopes() {
    const scopes = [];
    const base = vscode.workspace.getConfiguration('semanticus');
    const info = base.inspect('licenseToken');
    const globalValue = (info?.globalValue ?? '').trim();
    if (globalValue)
        scopes.push({ cfg: base, target: vscode.ConfigurationTarget.Global, label: 'user settings', value: globalValue });
    const workspaceValue = (info?.workspaceValue ?? '').trim();
    if (workspaceValue)
        scopes.push({ cfg: base, target: vscode.ConfigurationTarget.Workspace, label: 'workspace settings', value: workspaceValue });
    ...
    if (vscode.workspace.workspaceFile !== undefined) {
        for (const folder of vscode.workspace.workspaceFolders ?? []) {
            ...
            const folderValue = (fcfg.inspect('licenseToken')?.workspaceFolderValue ?? '').trim();
            if (folderValue)
                scopes.push({ cfg: fcfg, target: vscode.ConfigurationTarget.WorkspaceFolder, label: `workspace folder '${folder.name}'`, value: folderValue, uri: folder.uri });
        }
    }
    return scopes;
}
...
function freshValueAt(scope) {
    const info = vscode.workspace.getConfiguration('semanticus', scope.uri).inspect('licenseToken');
...
function reportKeptScopes(kept) {
    if (kept.length === 0)
        return;
    const labels = kept.map((s) => s.label).join(', ');
    vscode.window.showWarningMessage(`Semanticus kept ${labels}: ${kept.length === 1 ? 'it holds' : 'they hold'} a different license token than the one being cleaned up. If that token is stale, open that settings.json and delete the "semanticus.licenseToken" entry yourself.`);
}
...
async function clearLegacyScopes(expected) {
    const result = { expectedCleared: [], keptOther: [] };
    if (!expected)
        return result; // never treat "clear anything" as a valid request
    ...
    for (const scope of scopes) {
        ...
        try {
            await scope.cfg.update('licenseToken', undefined, scope.target);
        }
    ...
    for (const scope of attempted) {
        ...
        if (!after)
            result.expectedCleared.push(scope.label);
        else if (after === expected)
            result.keptOther.push({ scope: scope.label, why: 'could not be removed' });
        else
            result.keptOther.push({ scope: scope.label, why: 'its value changed while it was being cleared' });
    }
    if (result.expectedCleared.length > 0)
        licenseMutationRevision++; // something committed
    ...
    if (keptDifferent.length > 0) {
        vscode.window.showWarningMessage(`Semanticus kept ${keptDifferent.map((k) => `${k.scope} (${k.why})`).join(', ')}. If that license token is stale, open that settings.json and delete the "semanticus.licenseToken" entry yourself.`);
    }
    const failed = result.keptOther.filter((k) => k.why === 'could not be removed').map((k) => k.scope);
    if (failed.length > 0) {
        vscode.window.showWarningMessage(`Semanticus could not remove the plaintext license from ${failed.join(', ')}. Open that settings.json, delete the "semanticus.licenseToken" entry yourself, then run "Semanticus: Show license" to re-verify.`);
    }
    return result;
}
...
async function clearLegacyLicenseSetting(ask, expected) {
    if (!expected)
        return false;
    const scopes = definedLegacyScopes();
    ...
    if (ask) {
        if (!licenseMigrationNoticeShown) {
            ...
            const revisionAtPrompt = licenseMutationRevision;
            const epochAtPrompt = licenseMutationAttemptEpoch;
            void vscode.window.showInformationMessage(`Semanticus moved your Pro license into secure storage (the OS keychain). Remove the plaintext copy from settings.json (${scopeList})?`, 'Remove from settings', 'Keep').then((pick) => {
                if (pick !== 'Remove from settings')
                    return;
                void withLicenseMutation(async () => {
                    ...
                    const staleness = async () => {
                        ...
                        if (licenseMutationAttemptEpoch !== epochAtPrompt)
                            return 'attempted';
                        try {
                            return ((await extCtx.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim() !== expected ? 'committed' : 'fresh';
                        }
                        catch {
                            return 'unverified';
                        } // unreadable: cannot prove the keychain still holds this token
                    };
                    ...
                    if (verdict !== 'fresh') {
                        ...
                        vscode.window.showInformationMessage(verdict === 'committed'
...
function pipePathFor(pipeName) {
    ...
    if (bytes > limit)
        throw new Error(`Engine socket path is too long for a Unix domain socket (${bytes} bytes, limit ${limit} on ${process.platform}): ${p}. Set TMPDIR to a shorter path.`);
    return p;
}
...
function tryConnectPipe(pipeName) {
    return new Promise((resolve) => {
        let settled = false;
        let socket;
        const finish = (reason) => { if (settled)
            return; settled = true; try {
            socket?.destroy();
        }
        catch { /* ignore */ } resolve(reason); };
        socket = net.connect(pipePathFor(pipeName));
        socket.once('connect', () => finish(null));
        socket.once('error', (e) => finish(e?.code ?? e?.message ?? 'error'));
...
async function ensureEngine(ws, context, uiChallenge) {
    ...
    if (licenseToken)
        serveArgs.push('--license', licenseToken);
    let spawnCmd;
    let spawnArgs;
    if (engine.kind === 'dll') {
        ...
        if (process.platform !== 'win32') {
            try {
                fs.chmodSync(engine.path, 0o755);
            }
            catch { /* best effort */ }
        }
    ...
    for (let i = 0; i < 200; i++) {
        ...
        if (!info) {
            if (i % 25 === 0)
                out.appendLine(`  waiting… ${lastReason}`);
            continue;
        }
...
function renderHealthChip(h) {
    if (!healthStatus) {
        return;
    }
    if (!h) {
        lastHealthWorsened = false;
        healthStatus.hide();
        return;
    }
    ...
    const parts = [];
    if (issues > 0) {
        parts.push(`added ${issues} issue${issues === 1 ? '' : 's'}`);
    }
    if (things > 0) {
        parts.push(`affects ${things} thing${things === 1 ? '' : 's'}`);
    }
...
async function healthChipActionCmd() {
    if (!lastHealthWorsened) {
        await vscode.commands.executeCommand('semanticus.scanReadiness');
        return;
    }
    const undoItem = { label: '$(discard) Undo this change', detail: 'Revert the change that just worsened model health' };
    const scanItem = { label: '$(pulse) Open readiness scan', detail: 'See the full picture in the AI Readiness tab' };
    ...
    if (pick === undoItem) {
        await vscode.commands.executeCommand('semanticus.undo');
    }
    else if (pick === scanItem) {
        await vscode.commands.executeCommand('semanticus.scanReadiness');
    }
}
...
function scheduleEngineReconnect(context) {
    if (deactivating || reconnectTimer)
        return;
...
async function connectEngine(context, quiet = false) {
    const epoch = connectionEpoch;
    if (connectInFlight?.epoch === epoch)
...
async function connectEngineAttempt(context, quiet, epoch) {
    let connected;
    try {
        ...
        connected.onNotification('model/didChange', (n) => {
            ...
            try {
                studioPanel?.webview.postMessage({ type: 'didChange', payload: n });
            }
            catch { /* disposing */ }
        });
        ...
        connected.onNotification('spec/didChange', (v) => {
            try {
                studioPanel?.webview.postMessage({ type: 'specDidChange', payload: v });
            }
            catch { /* disposing */ }
        });
        ...
        connected.onNotification('layout/didChange', (v) => {
            try {
                studioPanel?.webview.postMessage({ type: 'layoutDidChange', payload: v });
            }
            catch { /* disposing */ }
        });
        ...
        connected.onNotification('workflow/didChange', (v) => {
            try {
                studioPanel?.webview.postMessage({ type: 'workflowDidChange', payload: v });
            }
            catch { /* disposing */ }
        });
        ...
        connected.onNotification('workflow/libraryDidChange', (v) => {
            try {
                studioPanel?.webview.postMessage({ type: 'workflowLibraryDidChange', payload: v });
            }
            catch { /* disposing */ }
        });
        ...
        connected.onNotification('model/activity', (e) => {
            try {
        ...
        connected.onClose(() => {
            if (conn !== connected)
                return; // an explicit restart already replaced/invalidated this connection
            conn = undefined;
            connectionEpoch++;
        ...
        let info = await connected.sendRequest('sessionInfo');
        if (info?.sessionId) {
            setStatusFromInfo(info);
        }
        else {
            const configured = vscode.workspace.getConfiguration('semanticus').get('modelPath');
            if (configured) {
                await connected.sendRequest('open', configured);
        ...
        if (info?.sessionId)
            await propGrid.showModelIfEmpty(info.modelName);
        else
            await propGrid.clear();
        tree.refresh();
        try {
        ...
        if (!quiet)
            vscode.window.showErrorMessage('Semanticus could not start its engine. See the Semanticus output channel. ' + (err?.message ?? err));
        scheduleEngineReconnect(context);
    }
}
...
async function restartEngineCmd(context) {
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Semanticus: restarting engine' }, async (progress) => {
        cancelScheduledReconnect();
        reconnectAttempt = 0;
        connectionEpoch++;
        const previous = conn;
        ...
        if (rebuild) {
            ...
            if (!ok)
                vscode.window.showWarningMessage('Semanticus: engine rebuild failed; restarting the previous build. See the Semanticus output.');
        }
...
async function activateLicenseCmd(context) {
    if (licenseInteractiveFlowActive) {
    ...
    try {
        await activateLicenseFlow(context);
    }
    finally {
        licenseInteractiveFlowActive = false;
    }
}
...
async function activateLicenseFlow(context) {
    ...
    const opened = await withLicenseMutation(async () => {
        let secretAtOpen = '';
        try {
            secretAtOpen = ((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim();
        }
        catch {
            secretAtOpen = null;
        }
        const legacyAtOpen = (vscode.workspace.getConfiguration('semanticus').get('licenseToken') || '').trim();
    ...
    if (token === undefined)
        return; // user cancelled — leave the setting untouched
    ...
    if (!trimmed) {
        const res = await withLicenseMutation(async () => {
            ...
            if (licenseMutationRevision !== opened.revision)
                return { kind: 'superseded-committed' };
            if (licenseMutationAttemptEpoch !== opened.epoch)
                return { kind: 'superseded-attempted' };
            let current = null;
            try {
                current = ((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim();
            }
            catch {
                current = null;
            }
            ...
            if (current === null) {
                ...
                try {
                    ...
                    if (secretGone)
                        licenseMutationRevision++; // the delete committed
                }
                catch (e) {
                    secretGone = false;
        ...
        if (res.kind === 'superseded-committed') {
            ...
            vscode.window.showWarningMessage('Semanticus: the license was cleared from secure storage' + (res.plaintextRemoved ? ' and its plaintext copy was removed' : '') +
            ...
            vscode.window.showWarningMessage('Semanticus: the license could not be fully cleared from secure storage' +
    ...
    const attempt = await withLicenseMutation(async () => {
        ...
        if (licenseMutationRevision !== opened.revision)
            return { superseded: 'committed' };
        if (licenseMutationAttemptEpoch !== opened.epoch)
            return { superseded: 'attempted' };
        let secretNow = '';
        try {
            secretNow = ((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim();
        }
        ...
        if (secretNow === null || opened.secretAtOpen === null) {
            ...
            if (secretNow !== opened.secretAtOpen)
                return { superseded: 'unverified' };
        }
        else if (secretNow !== opened.secretAtOpen) {
            return { superseded: 'committed' };
        }
        const legacyNow = (vscode.workspace.getConfiguration('semanticus').get('licenseToken') || '').trim();
        if (legacyNow !== opened.legacyAtOpen)
            return { superseded: 'committed' };
        let priorSecret = '';
        let priorSecretKnown = true;
        try {
            priorSecret = (await context.secrets.get(LICENSE_SECRET_KEY)) ?? '';
        }
        catch {
            priorSecretKnown = false;
        }
    ...
    if (attempt.superseded) {
        vscode.window.showInformationMessage(attempt.superseded === 'committed'
    ...
    if (attempt.stored) {
        ...
        const choice = await vscode.window.showWarningMessage('Semanticus could not save your license to secure storage (the OS keychain is unavailable on this machine).', {
            modal: true,
        ...
        if (choice !== 'Store in plaintext') {
            const rb = await withLicenseMutation(async () => {
                ...
                if (revTripped || licenseMutationAttemptEpoch !== attempt.epoch) {
                    ...
                    try {
                        const now = ((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim();
                        if (now !== trimmed)
                            return revTripped ? 'superseded-gone-committed' : 'superseded-gone-attempted';
                        return revTripped ? 'superseded-present-committed' : 'superseded-present-attempted';
                    }
                    catch {
                        return revTripped ? 'superseded-present-committed' : 'superseded-present-attempted';
                    } // unreadable — absence cannot be proven
                }
                if (!attempt.priorSecretKnown)
                    return 'uncertain'; // no coherent snapshot — never guess at a restore
                let current = null;
                try {
                    current = (await context.secrets.get(LICENSE_SECRET_KEY)) ?? '';
                }
                catch {
                    current = null;
                }
                if (current === null)
                    return 'uncertain';
                if (current === attempt.priorSecret)
                    return 'clean'; // prior state intact — nothing to undo
                ...
                try {
                    ...
                    if (attempt.priorSecret)
                        await context.secrets.store(LICENSE_SECRET_KEY, attempt.priorSecret);
                    else
                        await context.secrets.delete(LICENSE_SECRET_KEY);
                    ...
                    if (restored)
                        licenseMutationRevision++; // the rollback itself is a commit
        ...
        const wrote = await withLicenseMutation(async () => {
            if (licenseMutationRevision !== attempt.revision)
                return 'committed';
            if (licenseMutationAttemptEpoch !== attempt.epoch)
                return 'attempted';
            let secretNow = null;
            try {
                secretNow = ((await context.secrets.get(LICENSE_SECRET_KEY)) ?? '').trim();
            ...
            if (secretNow !== null && secretNow !== attempt.priorSecret.trim() && secretNow !== trimmed)
                return 'committed';
            const legacyNow = (vscode.workspace.getConfiguration('semanticus').get('licenseToken') || '').trim();
            if (legacyNow !== opened.legacyAtOpen)
                return 'committed';
        ...
        if (wrote !== 'written') {
            vscode.window.showInformationMessage(wrote === 'committed'
    ...
    if (spawned) {
        ...
        const pick = await vscode.window.showInformationMessage('License saved. Reload the window to activate it (Semanticus can\'t finish here on its own).', 'Reload Window');
...
async function manageLicenseCmd(known) {
    let info = known;
    if (!info && conn) {
        try {
            info = await conn.sendRequest('getEntitlement');
        }
    ...
    if (!opened)
        vscode.window.showWarningMessage('Semanticus: your browser did not open. Try “Semanticus: Pro Plans and Support” again.');
}
...
async function showLicenseCmd() {
    if (!conn) {
        vscode.window.showWarningMessage('Semanticus: not connected yet. Wait for the status bar to show your model, then try again.');
        return;
    }
    let info;
    try {
        ...
        vscode.window.showErrorMessage(`Semanticus: could not read the entitlement: ${e?.message ?? e}`);
        return;
    }
    if ((info?.tier ?? 'free').toLowerCase() === 'pro') {
        ...
        if (pick === 'Pro options')
            await manageLicenseCmd(info);
    }
    else {
        // Free — surface the engine's Reason VERBATIM (it teaches: no license / invalid / expired-past-grace + renew URL).
        const pick = await vscode.window.showInformationMessage(`Semanticus is on the free tier. ${info?.reason || 'No Pro license is active.'}`, 'Upgrade to Pro');
...
function killEngine(ws) {
    ...
    for (const f of ['engine.json', 'engine.lock']) {
        try {
            fs.rmSync(path.join(ws, '.semanticus', f));
        }
        catch { /* may not exist */ }
    }
}
...
function buildEngine() {
    return new Promise((resolve) => {
        ...
        if (!csproj) {
            out.appendLine('restart: cannot locate Semanticus.Engine.csproj to build (set semanticus.engineDll).');
            resolve(false);
            return;
        }
        ...
        proc.on('error', (e) => { out.appendLine('build spawn error: ' + (e?.message ?? e)); resolve(false); });
...
class ModelTreeProvider {
    ...
    getTreeItem(n) {
        ...
        if (n.kind === 'dfolder')
            item.tooltip = 'Display folder: a label on its members, not a container (it lives in each member’s Display Folder property)';
        return item;
    }
    async getChildren(n) {
        if (!this.connection)
        ...
        try {
            if (!n)
                return await this.connection.sendRequest('listTree', null);
            ...
            if (n.kind === 'table' || n.kind === 'calcgroup') {
                const kids = await this.connection.sendRequest('listTree', n.ref);
                this.flat.set(n.ref, kids);
                return (0, folders_1.groupFolderLevel)(kids, refParts(n.ref).name, '');
            }
            if (n.kind === 'dfolder') {
                const { table, path } = (0, folders_1.folderParts)(n.ref);
                return (0, folders_1.groupFolderLevel)(await this.tableChildren('table:' + table), table, path);
            }
            return await this.connection.sendRequest('listTree', n.ref);
        }
        catch (e) {
    ...
    async tableChildren(tableRef) {
        const hit = this.flat.get(tableRef);
        if (hit)
            return hit;
        if (!this.connection)
            return [];
        const kids = await this.connection.sendRequest('listTree', tableRef);
    ...
    async getParent(n) {
        const mk = (ref, name, kind) => ({ ref, name, kind, hasChildren: true });
        if (n.kind === 'dfolder') {
            const { table, path } = (0, folders_1.folderParts)(n.ref);
            const parent = (0, folders_1.parentFolderPath)(path);
            return parent ? mk((0, folders_1.folderRef)(table, parent), (0, folders_1.leafFolderName)(parent), 'dfolder') : mk('table:' + table, table, 'table');
        }
        ...
        switch (p.kind) {
            ...
            case 'hierarchy': {
                ...
                let df = '';
                try {
                    df = (0, folders_1.normFolder)((await this.tableChildren('table:' + p.table)).find((x) => x.ref === n.ref)?.displayFolder);
                }
                catch { /* engine hiccup — fall back to the table below */ }
                if (df)
                    return mk((0, folders_1.folderRef)(p.table, df), (0, folders_1.leafFolderName)(df), 'dfolder');
...
class ReferenceTreeProvider {
    constructor() {
        this._emitter = new vscode.EventEmitter();
        this.onDidChangeTreeData = this._emitter.event;
        this.nodes = [];
    }
    setNodes(nodes) { this.nodes = nodes; this._emitter.fire(); }
    ...
    getTreeItem(n) {
        ...
        item.iconPath = iconFor(n.kind);
        if (n.ref.includes('/'))
            item.tooltip = 'Copy into the open model (right-click, or Ctrl+C → Ctrl+V in the Model tree)';
        return item;
    }
    getChildren(n) {
        if (!n)
            return this.nodes.filter((x) => !x.ref.includes('/')); // top level = tables (no '/')
...
async function setReferenceModelCmd() {
    if (!conn) {
        void vscode.window.showWarningMessage('Open a model first, then set a reference model to copy from.');
        return;
    }
    const pick = await vscode.window.showQuickPick([{ label: '$(file) File…', detail: 'A .bim / .pbip / TMDL folder on disk', refKind: 'file' },
        { label: '$(git-branch) Git ref…', detail: "A commit / branch / tag of the open model's repo", refKind: 'gitref' },
        { label: '$(cloud) Workspace (XMLA)…', detail: 'A published Power BI / Fabric / AAS model', refKind: 'workspace' }], { placeHolder: 'Browse a model to copy objects FROM' });
    if (!pick)
        return;
    let ref;
...
function clearReferenceModel() {
    ...
    if (lastCopyFrom === 'reference')
        lastCopyFrom = treeCopyStash ? 'model' : undefined;
...
function copyRefStash() {
    if (!referenceRef)
        return;
    const sel = referenceView.selection.filter((x) => x.ref.includes('/')); // leaf objects only (measures / columns / items)
    if (sel.length === 0) {
        void vscode.window.showInformationMessage('Select a measure / column / calculation item in the Reference Model tree, then Ctrl+C.');
        return;
    }
...
async function pasteIntoModelCmd() {
    if (!copyStash) {
        void vscode.window.showInformationMessage('Nothing copied. Select objects in the Reference Model tree and press Ctrl+C first.');
        return;
    }
    await cherryPickInto(copyStash.source, copyStash.refs);
}
async function copyRefIntoModel(nodes) {
    if (!referenceRef)
        return;
    const refs = nodes.filter((n) => n?.ref?.includes('/')).map((n) => n.ref);
    if (refs.length === 0) {
        void vscode.window.showInformationMessage('Pick a measure / column / calculation item to copy.');
        return;
    }
    await cherryPickInto(referenceRef, refs);
}
async function cherryPickInto(source, refs) {
    if (!conn)
        return;
    try {
        const r = await conn.sendRequest('cherryPick', source, refs, true, true, 'human');
        if (r?.error) {
...
async function revealRefInTree(ref) {
    if (!treeView)
        return;
    const p = refParts(ref);
    const node = { ref, name: p.name, kind: p.kind, hasChildren: false };
    try {
        ...
        if (parent) {
            try {
                await treeView.reveal(parent, { select: false, focus: true, expand: true });
            }
            catch { /* give up */ }
        }
    }
...
function displayDax(raw) {
    if (!raw || raw.includes('\n'))
        return raw;
    try {
        const maxLine = vscode.workspace.getConfiguration('semanticus').get('dax.maxLineLength', 60);
        return (0, daxFormat_1.formatDax)(raw, { indent: '    ', maxLine });
    }
    catch {
        return raw;
    }
...
async function findInModelCmd(context) {
    if (!conn) {
        vscode.window.showWarningMessage('Connect to a model first.');
        return;
    }
    ...
    const toSearchTab = { iconPath: new vscode.ThemeIcon('replace-all'), tooltip: 'Open in Search & Replace' };
    qp.buttons = [toSearchTab];
    qp.onDidTriggerButton((b) => { if (b === toSearchTab) {
        const q = qp.value.trim();
        qp.hide();
        navigateStudio(context, 'search', q || undefined);
    } });
    let seq = 0;
    qp.onDidChangeValue(async (val) => {
        ...
        qp.busy = true;
        try {
            const res = await conn.sendRequest('searchModel', q, 200);
            if (mine !== seq)
        ...
        catch { /* transient (model closing) — ignore */ }
        finally {
            if (mine === seq)
                qp.busy = false;
        }
    ...
    qp.onDidAccept(async () => {
        ...
        if (!hit)
            return;
        const node = { ref: hit.ref, name: hit.name, kind: hit.kind, hasChildren: false };
        if (DAX_KINDS.has(hit.kind))
            await openDax(node);
        else {
            await propGrid.showObjects([node]);
...
function iconForKind(kind) {
    switch (kind) {
        ...
        case 'column':
        case 'calcColumn': return 'symbol-field';
        case 'table':
        case 'calcgroup': return 'table';
...
class DaxFileSystem {
    ...
    createDirectory() { }
    delete() { throw vscode.FileSystemError.NoPermissions('delete not supported'); }
    rename() { throw vscode.FileSystemError.NoPermissions('rename not supported'); }
}
let daxSymbols = { tables: [], measures: [], columns: [], functions: [] };
let daxIndex = null; // lowercased lookup index for the linter (null until a model loads)
let daxSymbolTimer;
function scheduleDaxSymbolRebuild() {
    if (daxSymbolTimer)
        clearTimeout(daxSymbolTimer);
...
async function rebuildDaxSymbols() {
    ...
    try {
        ...
        for (const t of tnodes) {
            const kids = await conn.sendRequest('listTree', t.ref);
            for (const k of kids) {
                if (k.kind === 'measure')
                    syms.measures.push({ name: k.name, ref: k.ref });
                else if (k.kind === 'column' || k.kind === 'calcColumn')
                    syms.columns.push({ table: t.name, name: k.name, ref: k.ref, kind: k.kind });
            }
        }
        daxSymbols = syms;
...
const daxCompletionProvider = {
    provideCompletionItems(doc, pos) {
        ...
        for (const v of (0, daxLint_1.collectVars)(doc.getText(new vscode.Range(new vscode.Position(0, 0), pos)))) {
            const it = new vscode.CompletionItem(v, vscode.CompletionItemKind.Variable);
            it.detail = 'variable';
            it.sortText = '0_' + v; // surface vars above functions
            items.push(it);
        }
        ...
        for (const fn of DAX_FUNCTIONS) {
            ...
            if (sig) {
                ...
                md.appendCodeblock(`${fn}(${sig.params.join(', ')})`, 'dax');
                md.appendMarkdown('\n' + sig.doc);
                it.documentation = md;
            }
            items.push(it);
...
class ModelTreeDnd {
    ...
    handleDrag(source, dt) {
        ...
        dragTablesStash = [...new Set(source.map(tableOfNode).filter((t) => !!t))];
    }
    handleDrop() { }
}
...
function isDaxDoc(doc) { return doc.languageId === 'dax'; }
...
function revalidateOpenDaxDocs() { for (const d of vscode.workspace.textDocuments)
    validateDaxDoc(d); }
const daxValidateTimers = new Map();
function scheduleDaxValidate(doc) {
    if (!isDaxDoc(doc))
        return;
...
function parseDaxRef(token) {
    let m = /^'((?:[^']|'')*)'\s*\[([^\]]*)\]$/.exec(token); // 'Table'[Col]
    if (m)
        return { kind: 'column', table: m[1].replace(/''/g, "'"), name: m[2] };
    m = /^([A-Za-z_]\w*)\[([^\]]*)\]$/.exec(token); // Table[Col]
    if (m)
        return { kind: 'column', table: m[1], name: m[2] };
    m = /^'((?:[^']|'')*)'$/.exec(token); // 'Table'
    if (m)
        return { kind: 'table', name: m[1].replace(/''/g, "'") };
    m = /^\[([^\]]*)\]$/.exec(token); // [Name]
    if (m)
        return { kind: 'measure', name: m[1] };
    return undefined;
}
const eqi = (a, b) => a.toLowerCase() === b.toLowerCase();
const daxHoverProvider = {
    async provideHover(doc, pos) {
        ...
        if (refRange) {
            ...
            if (ref?.kind === 'measure') {
                ...
                if (meas) {
                    ...
                    try {
                        const dax = await conn?.sendRequest('getDax', meas.ref);
                        if (dax)
                            md.appendCodeblock(dax.trim(), 'dax');
                    }
                    catch { /* offline */ }
                ...
                if (col)
                    return new vscode.Hover(new vscode.MarkdownString(`**${col.name}** · column · \`${col.table}\``), refRange);
            }
                ...
                if (col) {
                    ...
                    try { // enrich with the column's data type + display folder (isolated per-hover read)
                        const props = await conn?.sendRequest('getObjectProperties', col.ref);
                        const dt = props?.find((p) => p.name === 'DataType')?.value;
                        const df = props?.find((p) => p.name === 'DisplayFolder')?.value;
                        if (dt)
                            md.appendMarkdown(`\nType: \`${dt}\``);
                        if (df)
                            md.appendMarkdown(` · Folder: \`${df}\``);
                    }
                    catch { /* offline */ }
                    if (col.kind === 'calcColumn') {
                        try {
                            const dax = await conn?.sendRequest('getDax', col.ref);
                            if (dax)
                                md.appendCodeblock(dax.trim(), 'dax');
                        }
                        catch { /* offline */ }
                ...
                if (daxSymbols.tables.some((t) => eqi(t, ref.name)))
                    return new vscode.Hover(new vscode.MarkdownString(`**${ref.name}** · table`), refRange);
            }
        }
        ...
        if (wordRange) {
            ...
            const def = (0, daxLint_1.varDefinition)(doc.getText(), bare);
            if (def !== null) {
            ...
            if (DAX_FUNCTIONS.includes(word))
                return new vscode.Hover(new vscode.MarkdownString(`\`${word}\` · DAX function`), wordRange);
        }
        return undefined;
    },
};
...
const daxDefinitionProvider = {
    async provideDefinition(doc, pos) {
        ...
        if (wordRange) {
            ...
            const decl = (0, daxLint_1.extractDaxSymbols)(doc.getText()).find((s) => s.kind === 'var' && eqi(s.name, word));
            if (decl)
                return new vscode.Location(doc.uri, new vscode.Range(doc.positionAt(decl.nameStart), doc.positionAt(decl.nameEnd)));
        }
        return undefined;
    },
};
...
async function resolveDaxDefinition(ref) {
    const top = new vscode.Position(0, 0);
    if (ref.kind === 'measure') {
        const meas = daxSymbols.measures.find((x) => eqi(x.name, ref.name));
        if (meas)
            return new vscode.Location(uriForRef(meas.ref), top);
        const cc = daxSymbols.columns.find((x) => eqi(x.name, ref.name) && x.kind === 'calcColumn');
        if (cc)
            return new vscode.Location(uriForRef(cc.ref), top);
    }
    else if (ref.kind === 'column') {
    ...
    if (conn && ref.name) {
        try {
            const res = await conn.sendRequest('searchModel', ref.name, 50);
            ...
            if (hit)
                return new vscode.Location(uriForRef(hit.ref), top);
        }
...
const daxDocumentSymbolProvider = {
    provideDocumentSymbols(doc) {
        return (0, daxLint_1.extractDaxSymbols)(doc.getText()).map((s) => new vscode.DocumentSymbol(s.name, s.kind === 'var' ? 'variable' : '', s.kind === 'var' ? vscode.SymbolKind.Variable : vscode.SymbolKind.Key, new vscode.Range(doc.positionAt(s.fullStart), doc.positionAt(s.fullEnd)), new vscode.Range(doc.positionAt(s.nameStart), doc.positionAt(s.nameEnd))));
    },
};
// ---- DAX formatting --------------------------------------------------------------------------
...
function daxFormatOpts(options) {
    return { indent: options.insertSpaces ? ' '.repeat(options.tabSize) : '\t', maxLine: vscode.workspace.getConfiguration('semanticus').get('dax.maxLineLength', 60) };
}
...
const daxRangeFormatProvider = {
    provideDocumentRangeFormattingEdits(doc, range, options) {
        const text = doc.getText(range);
        const formatted = (0, daxFormat_1.formatDax)(text, daxFormatOpts(options));
...
async function callDaxFormatter(dax) {
    ...
    if (!resp.ok)
        throw new Error(`daxformatter.com returned HTTP ${resp.status}`);
...
async function formatDaxOnlineCmd(context) {
    const ed = vscode.window.activeTextEditor;
    if (!ed || ed.document.languageId !== 'dax') {
        vscode.window.showWarningMessage('Open a DAX expression to format.');
        return;
    }
    if (!context.globalState.get('semanticus.daxOnlineConsent')) {
        const pick = await vscode.window.showWarningMessage('Format with DAX Formatter sends this expression to the online service daxformatter.com (SQLBI). The expression leaves your machine. Continue?', { modal: true }, 'Use once', 'Always use');
        if (!pick)
            return;
        if (pick === 'Always use')
            await context.globalState.update('semanticus.daxOnlineConsent', true);
    }
    const text = ed.document.getText();
    try {
        const r = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Formatting DAX via daxformatter.com…' }, () => callDaxFormatter(text));
        if (r.errors && r.errors.length) {
            vscode.window.showErrorMessage('DAX Formatter: ' + r.errors.map((e) => `(${e.line},${e.column}) ${e.message}`).join('; '));
            return;
        }
        if (!r.formatted) {
            vscode.window.showErrorMessage('DAX Formatter returned no result.');
            return;
        }
        await ed.edit((b) => b.replace(fullRange(ed.document), r.formatted));
    }
    catch (e) {
        vscode.window.showErrorMessage('DAX Formatter failed: ' + (e?.message ?? e));
    }
}
// ---- commands --------------------------------------------------------------------------------
...
function setStatusFromInfo(info) {
    ...
    if (info?.sessionId !== healthSessionId) {
...
function renderSyncChip(info) {
    ...
    const short = (p) => { const t = (p || '').replace(/[\\/]+$/, ''); const i = Math.max(t.lastIndexOf('/'), t.lastIndexOf('\\')); return i >= 0 ? t.slice(i + 1) : t; };
    ...
    let warn = false, detail;
    if (!connected)
        detail = 'No live connection, so queries are unavailable.';
    else if (info.hasUnsavedChanges) {
        warn = true;
        detail = "Unsaved edits are not in the model you're querying.";
    }
    else if (same)
        detail = 'Editing and querying the same instance.';
    ...
    else if (info.liveKind === 'local')
        detail = 'Querying an attached local engine (Power BI Desktop).';
    else
        detail = 'Querying the published model. Reflects the last deploy.';
...
async function refreshSyncChip() {
    ...
    try {
        renderSyncChip(await conn.sendRequest('sessionInfo'));
    }
    catch { /* leave the chip as-is */ }
}
async function refreshStatus() {
    if (!conn)
        return undefined;
    try {
        const info = await conn.sendRequest('sessionInfo');
        setStatusFromInfo(info);
        return info;
    }
    catch {
...
async function openModelCommand(tree) {
    ...
    if (local.length) {
        ...
        items.push(...local.map((r) => ({ record: r, label: `$(vm) ${r.modelName || r.database || r.endpoint}`, description: r.label || 'local', detail: `${r.endpoint}${r.database ? ` · ${r.database}` : ''}` })));
    }
    if (xmla.length) {
...
async function openFromFile() {
    ...
    if (!picked || picked.length === 0)
        return undefined;
    return conn.sendRequest('open', picked[0].fsPath);
}
async function openFromLocal() {
    const instances = (await conn.sendRequest('listLocalInstances').catch(() => [])) ?? [];
    ...
    if (instances.length === 1) {
        ...
        const p = await vscode.window.showQuickPick(instances.map((i) => ({ label: i.title || i.dataSource, detail: i.dataSource, ds: i.dataSource })), { placeHolder: 'Pick a running local model' });
        if (!p)
            return undefined;
        ds = p.ds;
    }
    return vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Semanticus: opening local model…' }, () => conn.sendRequest('openLocal', ds, null));
}
...
async function migrateLegacyXmlaHistory() {
    const legacy = extCtx?.globalState.get(LEGACY_RECENT_XMLA_KEY) ?? [];
    if (!legacy.length || !conn || !extCtx)
        return;
    const connection = conn;
    const context = extCtx;
    try {
        const migrated = await (0, legacyXmlaMigration_1.migrateLegacyXmlaEntries)(legacy, (r, mode) => connection.sendRequest('rememberXmlaConnection', r.endpoint, r.database || null, r.modelName || null, mode, 'human'), () => context.globalState.update(LEGACY_RECENT_XMLA_KEY, undefined));
...
async function openFromXmla() {
    ...
    const r = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Semanticus: connecting to XMLA…' }, () => conn.sendRequest('openLive', ep, db || null, am, null, null, authPick.forceReauth ?? false));
    return r;
}
async function saveCommand() {
    ...
    try {
        const r = await conn.sendRequest('save', null, 'TMDL');
...
async function saveToLiveCommand() {
    ...
    let dry;
    try {
        dry = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Semanticus: computing changes (dry run)…' }, () => conn.sendRequest('deployLive', null, null, null, null, null, false));
    }
    catch (e) {
        vscode.window.showErrorMessage('Save to live (dry run) failed: ' + (e?.message ?? e));
        return;
    }
    if (dry.error) {
        vscode.window.showErrorMessage('Save to live (dry run): ' + dry.error);
        return;
    }
    if (!dry.totalChanges) {
        vscode.window.showInformationMessage('No metadata changes to deploy. The live model already matches the session.');
        return;
    }
    ...
    const go = await vscode.window.showWarningMessage(`Deploy ${dry.totalChanges} metadata change(s) to the live model “${dry.database}”?`, { modal: true, detail: `Endpoint: ${dry.endpoint}\n\n${list}${more}${extra}\n\nMetadata only (incl. new measures / calc columns). No data refresh, no deletes. This writes to the LIVE model.` }, 'Deploy');
    if (go !== 'Deploy')
        return;
    // 3) COMMIT.
    try {
        const res = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Semanticus: deploying to the live model…' }, () => conn.sendRequest('deployLive', null, null, null, null, null, true));
        if (res.error)
            vscode.window.showErrorMessage('Deploy failed; nothing was committed: ' + res.error);
        else if (res.committed)
            vscode.window.showInformationMessage(`Deployed ${res.totalChanges} change(s) to “${res.database}”.`);
        else
            vscode.window.showWarningMessage(`Deploy made no changes (${res.totalChanges ?? 0} computed).`);
    }
    catch (e) {
        ...
        if (emsg.includes('blocked by the deploy gate')) {
            await overrideDeployToLive(emsg);
            return;
        }
...
async function overrideDeployToLive(gateMsg) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    ...
    if (!reason || !reason.trim())
        return;
    try {
        const res = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Semanticus: deploying to the live model (override)…' }, () => conn.sendRequest('deployLive', null, null, null, null, null, true, 'human', reason.trim()));
        if (res.error)
            vscode.window.showErrorMessage('Deploy failed; nothing was committed: ' + res.error);
        else if (res.committed)
            vscode.window.showWarningMessage(`Deployed ${res.totalChanges} change(s) to “${res.database}” (gate overridden).`);
        else
            vscode.window.showWarningMessage(`Deploy made no changes (${res.totalChanges ?? 0} computed).`);
    }
    catch (e) {
...
function semanticusMcpEntry(engine, ws, licenseToken) {
    ...
    if (licenseToken)
        args.push('--license', licenseToken);
    return { command, args };
}
...
function mergeMcpConfig(existing, entry) {
    ...
    const merged = isObj(existing) ? { ...existing } : {};
    const servers = isObj(merged.mcpServers) ? { ...merged.mcpServers } : {};
...
function mcpJsonIgnored(ws) {
    return new Promise((resolve) => {
        cp.execFile('git', ['check-ignore', '-q', '.mcp.json'], { cwd: ws }, (err) => {
            if (!err)
                return resolve(true); // exit 0 — ignored
            if (err.code === 1)
                return resolve(false); // exit 1 — tracked/untracked but NOT ignored
...
function mcpJsonTracked(ws) {
    return new Promise((resolve) => {
        cp.execFile('git', ['ls-files', '--error-unmatch', '.mcp.json'], { cwd: ws }, (err) => {
            if (!err)
                return resolve(true);
            if (err.code === 1)
...
async function warnIfMcpJsonNotIgnored(ws) {
    const key = `mcpGitignoreWarned:${ws}`;
    if (extCtx.globalState.get(key))
        return; // already handled for this workspace — never nag again
    if ((await mcpJsonIgnored(ws)) !== false)
        return; // ignored, or can't tell (no repo/git) — nothing to warn about
    ...
    if (tracked === null) {
        ...
        vscode.window.showWarningMessage('Semanticus wrote your Pro license token into .mcp.json but could not determine whether git is tracking that file (the git check did not complete). Before committing, verify that .mcp.json is git-ignored, or remove the token from it.');
        return; // flag intentionally left unset — re-offered on the next Connect Claude Code
    }
    if (tracked === true) {
        const pick = await vscode.window.showWarningMessage('.mcp.json holds your Semanticus Pro license token and is already committed to this git repository, ' +
            'so the token is in your repo history. Adding it to .gitignore will not remove it. ' +
            'Use the git commands to stop tracking it and to clean the history.', 'Copy git commands', 'Not now');
        if (pick === 'Copy git commands') {
            try {
                ...
                vscode.window.showErrorMessage(`Could not copy the commands: ${e?.message ?? e}`);
            } // flag NOT set — re-offered next Connect
        }
    ...
    const pick = await vscode.window.showWarningMessage('.mcp.json now contains your Pro license token and is not git-ignored, so it could be committed to your repo. Add it to .gitignore?', 'Add to .gitignore', 'Not now');
    if (pick === 'Add to .gitignore') {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Could not update .gitignore: ${e?.message ?? e}`);
        } // flag NOT set — re-offered next Connect
    }
    else if (pick === 'Not now') {
...
async function connectClaudeCodeCmd() {
    const ws = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!ws) {
        vscode.window.showErrorMessage('Open a folder first. Semanticus writes the AI Assistant connection into your open folder.');
        return;
    }
    ...
    if (fs.existsSync(target)) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Could not read ${target}: ${e?.message ?? e}`);
            return;
        }
        let parsed;
        try {
        ...
        if (res.conflict) {
            const pick = await vscode.window.showWarningMessage(`${target} already has a different "semanticus" MCP server entry. Replace it?`, { modal: true, detail: 'The existing entry points elsewhere. Replace it to connect your AI Assistant to this Semanticus session and workspace, or keep it unchanged.' }, 'Replace', 'Keep existing');
            if (pick !== 'Replace')
                return;
        }
        merged = res.merged;
...
function selectionRefs(node, nodes) {
...
async function createAndRefresh(method, params, label, openInEditor = false, afterOrigin = []) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    try {
        ...
        const ref = await conn.sendRequest(method, ...params, 'human', ...afterOrigin);
        tree.refresh();
        // Land DAX authoring in the full Monaco editor (the semanticus: virtual file), never a tiny input box.
        if (openInEditor && ref)
            await openDax({ ref, name: refParts(ref).name, kind: refParts(ref).kind, hasChildren: false });
        return ref;
    }
    catch (e) {
...
async function authorNewRole() {
    ...
    if (!name || !conn)
        return;
    try {
        await conn.sendRequest('createRole', name, null, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Create role failed: ' + (e?.message ?? e));
    }
}
...
async function authorNewMeasure(n) {
    ...
    if (!n) {
        ...
        if (sel && (sel.kind === 'table' || sel.kind === 'calcgroup' || sel.kind === 'dfolder'))
            n = sel;
        else {
            const t = sel?.ref ? refParts(sel.ref).table : undefined;
            if (t)
                n = { ref: 'table:' + t, name: t, kind: 'table', hasChildren: true };
        }
        if (!n) {
            if (!conn) {
                warnNoEngine();
            ...
            let tables = [];
            try {
                tables = (await conn.sendRequest('listTree', null)).filter((x) => x.kind === 'table' || x.kind === 'calcgroup');
            }
            catch { /* engine hiccup — the empty-list message below covers it */ }
            if (!tables.length) {
                vscode.window.showInformationMessage('Open a model (with at least one table) to add a measure.');
                return;
            }
    ...
    let folder = '';
    if (n.kind === 'dfolder') {
        const fp = (0, folders_1.folderParts)(n.ref);
        folder = fp.path;
...
async function newFolderCmd(n) {
    ...
    if (!n?.ref) {
        vscode.window.showInformationMessage('Right-click a table (or a folder on one) to add a folder.');
        return;
    }
    const base = n.kind === 'dfolder' ? (0, folders_1.folderParts)(n.ref) : { table: refParts(n.ref).name, path: '' };
    const name = await vscode.window.showInputBox({
        prompt: `New folder on ${base.table}${base.path ? ` inside ${base.path}` : ''}`,
        placeHolder: 'e.g. KPIs; use \\ to nest (KPIs\\Growth)',
        validateInput: (v) => (0, folders_1.normFolder)(v) ? undefined : 'Type a folder name.',
    });
    if (!name)
        return;
...
async function moveToFolderCmd(node, nodes) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    ...
    if (!movable.length) {
        vscode.window.showInformationMessage('Select a measure, column, or hierarchy to move to a folder.');
        return;
    }
    const tables = [...new Set(movable.map((t) => refParts(t.ref).table).filter((t) => !!t))];
    // Folders in use on the involved tables — every ancestor level is a real target too ('A\B' implies 'A').
    const folders = new Map();
    for (const t of tables) {
        let kids = [];
        try {
            kids = await tree.tableChildren('table:' + t);
        }
        catch { /* engine hiccup — New folder… still offered */ }
        for (const k of kids) {
            let p = (0, folders_1.normFolder)(k.displayFolder);
            while (p) {
                if (!folders.has(p.toLowerCase()))
                    folders.set(p.toLowerCase(), p);
    ...
    const items = [
        ...[...folders.values()].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' })).map((f) => ({ label: '$(folder) ' + f, act: 'folder', path: f })),
    ...
    if (pick.act === 'folder')
        ...
        if (!name || !(0, folders_1.normFolder)(name))
            return;
        target = (0, folders_1.normFolder)(name);
    }
    try {
        await conn.sendRequest('setDisplayFolder', movable.map((t) => t.ref), target, 'human');
        tree.refresh();
        void vscode.window.setStatusBarMessage(target ? `$(folder) Moved ${what} into ${target}. One undo restores ${movable.length === 1 ? 'it' : 'them'}` : `$(folder) ${what} moved out of ${movable.length === 1 ? 'its folder' : 'their folders'}`, 5000);
    }
    catch (e) {
...
async function renameFolderCmd(n) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    n ??= treeView?.selection[0];
    if (!n || n.kind !== 'dfolder') {
        vscode.window.showInformationMessage('Right-click a folder in the Model tree to rename it.');
        return;
    }
    const { table, path } = (0, folders_1.folderParts)(n.ref);
    ...
    if (!newName || (0, folders_1.normFolder)(newName) === n.name)
        return;
    const to = (parent ? parent + '\\' : '') + (0, folders_1.normFolder)(newName);
    try {
        const r = await conn.sendRequest('renameDisplayFolder', 'table:' + table, path, to, 'human');
        tree.refresh();
...
async function hierarchyFromColumnsCmd(node, nodes) {
    ...
    if (!cols.length) {
        vscode.window.showWarningMessage('Select one or more columns of a single table first.');
        return;
    }
    const tables = new Set(cols.map((c) => refParts(c.ref).table));
    if (tables.size !== 1) {
        vscode.window.showWarningMessage('All columns must belong to the same table.');
        return;
    }
...
function copyObjectCmd(n) {
    if (!n) {
        // The Ctrl+C keybinding passes no node — copy the tree selection (one object in v1).
        const sel = treeView ? [...treeView.selection] : [];
        if (sel.length > 1) {
            void vscode.window.showInformationMessage('Copy works on one object at a time. Select a single object and copy again.');
            return;
        }
        n = sel[0];
    }
...
async function pasteObjectCmd(n) {
    ...
    if (copyStash && (lastCopyFrom === 'reference' || !treeCopyStash)) {
        await pasteIntoModelCmd();
        return;
    }
    if (!treeCopyStash) {
    ...
    const tTable = tKind === 'table' || tKind === 'calcgroup' ? refParts(target.ref).name
        : tKind === 'dfolder' ? (0, folders_1.folderParts)(target.ref).table
            : refParts(target.ref).table;
    const intoFolder = tKind === 'dfolder' ? (0, folders_1.folderParts)(target.ref).path : '';
    if (clip.kind === 'measure' || clip.kind === 'calcitem') {
        if (tTable && tTable !== clip.table)
            targetRef = 'table:' + tTable;
    }
    ...
    try {
        ...
        const newRef = await conn.sendRequest('duplicateObject', clip.ref, null, 'human', targetRef);
        ...
        if (intoFolder && newRef) {
            try {
                await conn.sendRequest('setDisplayFolder', [newRef], intoFolder, 'human');
            }
            catch (e) {
                void vscode.window.showInformationMessage(`Pasted, but could not file the copy into ${intoFolder}: ${e?.message ?? e}`);
            }
        }
        tree.refresh();
...
async function renameCmd(n) {
    ...
    if (!newName || newName === n.name)
        return;
    try {
        await conn.sendRequest('renameObject', n.ref, newName, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Rename failed: ' + (e?.message ?? e));
    }
}
async function authorDelete(node, nodes) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    ...
    try {
        for (const t of targets)
            await conn.sendRequest('deleteObject', t.ref, 'human');
        tree.refresh();
    }
    catch (e) {
...
async function scriptObjectsCmd(format, node, nodes) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    ...
    if (format === 'tmdl')
        refs = tmdlEditableRefs(refs);
    const lang = format === 'dax' ? 'dax' : format === 'tmsl' ? 'json' : 'tmdl';
    try {
        const text = await conn.sendRequest('scriptObjects', refs, format);
        const doc = await vscode.workspace.openTextDocument({ content: text, language: lang });
        await vscode.window.showTextDocument(doc, { preview: false });
    }
    catch (e) {
        vscode.window.showErrorMessage('Script failed: ' + (e?.message ?? e));
    }
}
...
function tmdlEditableRefs(refs) {
    const out = [];
    for (const r of refs) {
...
async function applyTmdlScriptCmd() {
    ...
    const ed = vscode.window.activeTextEditor;
    if (!ed) {
        vscode.window.showWarningMessage('Open a TMDL script (right-click ▸ Script ▸ TMDL) to apply.');
        return;
    }
    try {
        const r = await conn.sendRequest('applyTmdlScript', ed.document.getText(), 'human');
        tree.refresh();
        const a = r.applied?.length ?? 0, s = r.skipped?.length ?? 0;
        if (s)
            vscode.window.showWarningMessage(`Applied ${a} TMDL document(s); skipped ${s}: ${r.skipped.join('; ')}`);
        else
            vscode.window.showInformationMessage(`Applied ${a} TMDL document(s) to the model.`);
    }
    catch (e) {
        vscode.window.showErrorMessage('Apply TMDL failed: ' + (e?.message ?? e));
    }
}
...
async function applyDaxScriptCmd() {
    ...
    const ed = vscode.window.activeTextEditor;
    if (!ed) {
        vscode.window.showWarningMessage('Open a DAX script (right-click ▸ Script ▸ DAX) to apply.');
        return;
    }
    try {
        const r = await conn.sendRequest('applyDaxScript', ed.document.getText(), 'human');
        tree.refresh();
        const a = r.applied?.length ?? 0, s = r.skipped?.length ?? 0;
        if (s)
            vscode.window.showWarningMessage(`Applied ${a} expression(s); skipped ${s} (not found / not DAX): ${r.skipped.join(', ')}`);
        else
            vscode.window.showInformationMessage(`Applied ${a} DAX expression(s) to the model.`);
    }
    catch (e) {
        vscode.window.showErrorMessage('Apply DAX script failed: ' + (e?.message ?? e));
    }
}
/// Set IsHidden on the whole selection (skips objects that can't be hidden).
async function setHiddenCmd(hidden, node, nodes) {
    if (!conn) {
        warnNoEngine();
        return;
    }
    for (const t of selectionRefs(node, nodes)) {
        try {
            await conn.sendRequest('setObjectProperty', t.ref, 'IsHidden', String(hidden), 'human');
        }
        catch { /* not hideable */ }
    }
    tree.refresh();
}
async function showDependentsCmd(n) {
    if (!conn || !n)
        return;
    try {
        const deps = await conn.sendRequest('getDependents', n.ref);
        if (!deps.length) {
            vscode.window.showInformationMessage(`Nothing references ${n.name}.`);
            return;
        }
        const pick = await vscode.window.showQuickPick(deps.map((d) => ({ label: d.name, description: d.kind, detail: d.ref })), { title: `${deps.length} dependent(s) of ${n.name}. Pick to open its DAX`, matchOnDescription: true });
        if (pick?.detail) {
            try {
                await openDax({ ref: pick.detail, name: pick.label, kind: refParts(pick.detail).kind, hasChildren: false });
            }
            catch {
                vscode.window.showInformationMessage(pick.detail);
            }
        }
    }
    catch (e) {
        vscode.window.showErrorMessage('Show dependents failed: ' + (e?.message ?? e));
    }
}
async function revealProperties(n) {
    if (n)
        await propGrid.showObject(n);
    await vscode.commands.executeCommand('semanticusProperties.focus');
}
...
async function setFormatCmd(n) {
    if (!conn || !n)
        return;
    let templates = [];
    try {
        templates = await conn.sendRequest('listFormatTemplates', null, null);
    }
    ...
    if (templates.length) {
        ...
        for (const p of ['#,0', '#,0.00', '0.0%', '0.00%', '\\$#,0', '\\$#,0.00', '#,0;(#,0)', '0'])
            items.push({ label: p });
    }
    ...
    try {
        ...
        vscode.window.showErrorMessage('Set format failed: ' + (e?.message ?? e));
    }
}
async function timeIntelCmd(n) {
    if (!conn || !n)
        return;
    ...
    try {
        ...
        vscode.window.showErrorMessage('Time-intelligence failed: ' + (e?.message ?? e));
    }
}
// ---- Column-specific ----
async function summarizeByCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (!pick)
        return;
    try {
        await conn.sendRequest('setColumnMetadata', n.ref, null, pick, null, null, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Set summarize-by failed: ' + (e?.message ?? e));
    }
}
async function dataCategoryCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (!pick)
        return;
    try {
        await conn.sendRequest('setColumnMetadata', n.ref, null, null, pick, null, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Set data category failed: ' + (e?.message ?? e));
    }
}
// ---- Table-specific ----
async function markDateTableCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (dateCol === undefined)
        return;
    try {
        await conn.sendRequest('markDateTable', n.ref, dateCol || null, 'human');
        tree.refresh();
        vscode.window.showInformationMessage(`Marked ${n.name} as the date table.`);
    }
    catch (e) {
        vscode.window.showErrorMessage('Mark date table failed: ' + (e?.message ?? e));
    }
}
// ---- Partition-specific ----
...
async function refreshPartitionCmd(n) {
    if (!conn || !n || !n.ref?.startsWith('partition:'))
        return;
    let types;
    try {
        types = await conn.sendRequest('listRefreshTypes');
    }
    catch (e) {
        vscode.window.showErrorMessage('Could not load refresh types: ' + (e?.message ?? e));
        return;
    }
    const pick = await vscode.window.showQuickPick((types || []).filter(t => t.partitionLevel).map(t => ({
    ...
    let plan;
    try {
        plan = await conn.sendRequest('refreshPartition', n.ref, pick.type, null, null, null, null, null, false);
    }
    catch (e) {
        vscode.window.showErrorMessage('Refresh not available: ' + (e?.message ?? e));
        return;
    }
    const confirmLabel = pick.type === 'ClearValues' ? 'Clear values (destructive)' : `Run ${pick.type} refresh`;
    const ok = await vscode.window.showWarningMessage(plan?.plan ?? `Refresh ${n.name} with ${pick.type}?`, { modal: true, detail: `LIVE data write against ${plan?.endpoint ?? 'the live model'}${plan?.database ? ' / ' + plan.database : ''}.` }, confirmLabel);
    if (ok !== confirmLabel)
        return;
    // Commit — the actual live refresh.
    try {
        const res = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Refreshing ${n.name} (${pick.type})…`, cancellable: false }, () => conn.sendRequest('refreshPartition', n.ref, pick.type, null, null, null, null, null, true));
        if (res?.committed)
            vscode.window.showInformationMessage(`Refreshed ${n.name} (${pick.type}).`);
        else
            vscode.window.showErrorMessage(`Refresh failed; nothing changed: ${res?.error ?? 'unknown error'}`);
    }
    catch (e) {
        vscode.window.showErrorMessage('Refresh failed: ' + (e?.message ?? e));
    }
}
// ---- Relationship-specific ----
async function relCrossFilterCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (!pick)
        return;
    try {
        await conn.sendRequest('setRelationship', refParts(n.ref).name, pick, null, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Set cross-filter failed: ' + (e?.message ?? e));
    }
}
async function relToggleActiveCmd(n) {
    if (!conn || !n)
        return;
    const makeActive = n.name.includes('(inactive)'); // the tree labels inactive relationships
    try {
        await conn.sendRequest('setRelationship', refParts(n.ref).name, null, makeActive, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Toggle active failed: ' + (e?.message ?? e));
    }
}
// ---- Role-specific ----
async function roleModelPermCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (!pick)
        return;
    try {
        await conn.sendRequest('setRolePermission', n.name, pick, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Set permission failed: ' + (e?.message ?? e));
    }
}
async function roleAddRlsCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (filter === undefined)
        return;
    try {
        await conn.sendRequest('setTablePermission', n.name, table, filter, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Set RLS failed: ' + (e?.message ?? e));
    }
}
async function roleAddMemberCmd(n) {
    if (!conn || !n)
        return;
    ...
    if (!member)
        return;
    try {
        await conn.sendRequest('setRoleMember', n.name, member, true, 'human');
        tree.refresh();
    }
    catch (e) {
        vscode.window.showErrorMessage('Add member failed: ' + (e?.message ?? e));
    }
}
    ...
    for (const p of first) {
        ...
        for (let i = 1; i < all.length; i++) {
            ...
            if (q.value !== p.value)
                varies = true;
            if (q.readOnly)
                p.readOnly = true; // read-only on any → read-only for the batch
            if (q.suggestions?.length)
                p.suggestions = [...new Set([...(p.suggestions ?? []), ...q.suggestions])]; // union prefills
        }
        if (onAll)
            out.push({ ...p, varies, value: varies ? '' : p.value });
    }
    return out;
}
class PropertyGridProvider {
    constructor(getConn, extensionUri) {
        this.getConn = getConn;
                ...
                try {
                    ...
                    if (msg.name === 'FormatStringExpression') {
                        for (const ref of refs)
                            await conn.sendRequest('setMeasureFormatExpression', ref, msg.value, 'human');
                    }
                        ...
                        for (const ref of cols)
                            await conn.sendRequest('setColumnDataType', ref, msg.value, 'human');
                        if (rest.length)
                            await conn.sendRequest('setObjectProperties', rest, msg.name, msg.value, 'human');
                    }
    ...
    async pushTemplates() {
        ...
        if (!this.templates) {
            try {
                this.templates = await conn.sendRequest('listFormatTemplates', null, null);
            }
            catch {
                return;
            } // no catalog → the grid degrades to plain text inputs (no combo affordance)
        }
    ...
    async showObject(node) { await this.showObjects(node ? [node] : []); }
    ...
    async push() {
        ...
        try {
            const all = await Promise.all(refs.map((r) => conn.sendRequest('getObjectProperties', r).catch(() => [])));
...
function openStudio(context) {
    ...
    const panel = vscode.window.createWebviewPanel('semanticusStudio', 'Semanticus Studio', vscode.ViewColumn.Active, {
        enableScripts: true,
        retainContextWhenHidden: true,
        localResourceRoots: [vscode.Uri.joinPath(context.extensionUri, 'media')],
    });
    studioPanel = panel;
    panel.onDidDispose(() => { if (studioPanel === panel) {
        studioPanel = undefined;
        studioReady = false;
    } });
    // Relay: webview RPC requests -> the single engine connection -> result back to the webview.
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg?.type === 'revealInTree') {
        ...
        if (msg?.type === 'focusModelTree') {
            ...
            return;
        } // Ctrl+Alt+T in Studio — only the host can move focus out of a webview
        ...
        if (msg?.type === 'exportDoc') {
            void exportDocToFile(msg);
            return;
        } // Documentation tab "Export…"
        if (msg?.type === 'printDoc') {
            void printDocInBrowser(msg);
            return;
        } // Documentation tab "Print / PDF" (system browser)
        if (msg?.type === 'manageLicense') {
        ...
        if (msg?.type !== 'rpc')
            return;
        if (!conn) {
            panel.webview.postMessage({ type: 'rpcResult', id: msg.id, error: 'Engine not connected.' });
            return;
        }
        try {
            ...
            const result = params.length
                ? await conn.sendRequest(msg.method, node_1.ParameterStructures.byPosition, ...params)
            ...
            if (modelSwapped) {
                try {
                    await propGrid.showModel(result?.context?.editing?.modelName ?? result?.modelName ?? 'Model');
                }
                catch { }
                tree.refresh();
                await refreshStatus();
...
function navigateStudio(context, tab, target, addTables) {
...
function flushNav() {
    if (studioPanel && studioReady && pendingNav) {
        try {
            studioPanel.webview.postMessage({ type: 'navigate', tab: pendingNav.tab, target: pendingNav.target, addTables: pendingNav.addTables });
        }
        catch { /* disposing */ }
...
function addToDiagramCmd(context, node, nodes) {
    const sel = (nodes && nodes.length ? nodes : (node ? [node] : [...(treeView?.selection ?? [])]));
    const tables = [...new Set(sel.map(tableOfNode).filter((t) => !!t))];
    if (!tables.length) {
        vscode.window.showInformationMessage('Right-click a table (or a column/measure/hierarchy) to add it to the diagram.');
        return;
    }
...
function previewTableDataCmd(context, n) {
    if (!n?.ref)
        return;
...
const schemaDiffPanels = new Map();
async function updateSchemaCmd(context, node) {
    if (!conn) {
        void vscode.window.showWarningMessage('Open a model first.');
        return;
    }
    ...
    if (!tableRef) {
        ...
        let tables = [];
        try {
            tables = (await conn.sendRequest('listTree', null)).filter((t) => t.kind === 'table');
        }
        catch (e) {
            void vscode.window.showErrorMessage('Could not list tables: ' + (e?.message ?? e));
            return;
        }
        if (!tables.length) {
            void vscode.window.showInformationMessage('No tables to update.');
            return;
        }
        const pick = await vscode.window.showQuickPick(tables.map((t) => ({ label: t.name, ref: t.ref })), { title: 'Update Schema', placeHolder: 'Pick a table to re-read its source schema' });
        if (!pick)
            return;
        tableRef = pick.ref;
    }
...
function openSchemaDiffPanel(context, tableRef, tableName) {
    const existing = schemaDiffPanels.get(tableRef);
    if (existing) {
        existing.reveal(vscode.ViewColumn.Active);
        void loadSchemaDiff(existing, tableRef);
        return;
    }
    const panel = vscode.window.createWebviewPanel('semanticusSchemaDiff', `Update Schema: ${tableName}`, vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true });
    schemaDiffPanels.set(tableRef, panel);
    panel.onDidDispose(() => { if (schemaDiffPanels.get(tableRef) === panel)
        schemaDiffPanels.delete(tableRef); });
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (!conn) {
            panel.webview.postMessage({ type: 'error', error: 'Engine not connected.' });
            return;
        }
        if (msg?.type === 'ready' || msg?.type === 'refresh') {
            void loadSchemaDiff(panel, tableRef);
            return;
        }
        if (msg?.type === 'apply') {
            ...
            if (!items.length) {
                panel.webview.postMessage({ type: 'applied', result: { changed: false, added: 0, removed: 0, retyped: 0, skipped: [] } });
                return;
            }
            try {
...
async function loadSchemaDiff(panel, tableRef) {
    if (!conn)
        return;
    try {
...
function schemaDiffHtml(webview, tableName) {
    const nonce = getNonce();
    const csp = `default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src 'nonce-${nonce}';`;
    const title = tableName.replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
...
async function printDocInBrowser(msg) {
    try {
        const content = typeof msg?.content === 'string' ? msg.content : '';
        if (!content) {
            void vscode.window.showWarningMessage('Nothing to print yet. The documentation preview is still rendering.');
            return;
        }
...
async function exportDocToFile(msg) {
    try {
...
function studioHtml(webview, context) {
