...
exports.folderParts = folderParts;
exports.parentFolderPath = parentFolderPath;
exports.leafFolderName = leafFolderName;
exports.folderRef = folderRef;
exports.groupFolderLevel = groupFolderLevel;
...
function leafFolderName(path) {
    return path.slice(path.lastIndexOf('\\') + 1);
}
/** The dfolder ref for a table + normalized folder path. */
function folderRef(table, path) {
 ...
 */
function groupFolderLevel(kids, table, prefix) {
    ...
    for (const k of kids) {
        ...
        if (dfLower === prefixLower) {
            direct.push(k);
            continue;
        } // filed exactly here
        if (prefixLower ? !dfLower.startsWith(nestedLower) : false)
            continue; // a different branch
