...
namespace Semanticus.Tests
{
    ...
    public sealed class ConnectionAccountHistoryTests : IDisposable
    {
        ...
        {
            var r = ConnectionRegistry.Remember("xmla", "powerbi://x/acct", "DS", "DS", "contoso.com", "interactive", "megan@contoso.com");
            Assert.Equal("megan@contoso.com", r.LastAccount);
            Assert.Equal("megan@contoso.com", ConnectionRegistry.Find(r.Id).LastAccount);   // survived the write
        }

        [Fact]
        public void A_headless_reconnect_that_names_no_account_keeps_the_known_one()
        {
            var r = ConnectionRegistry.Remember("xmla", "powerbi://x/keepacct", "DS", "DS", "contoso.com", "interactive", "megan@contoso.com");
            ...
            Assert.Equal("megan@contoso.com", ConnectionRegistry.Find(r.Id).LastAccount);
        }

        [Fact]
        public void Signing_in_as_a_different_account_overwrites_the_hint()
        {
            var r = ConnectionRegistry.Remember("xmla", "powerbi://x/switch", "DS", "DS", "contoso.com", "interactive", "megan@contoso.com");
            ConnectionRegistry.Remember("xmla", "powerbi://x/switch", "DS", "DS", "fabrikam.com", "interactive", "avery@fabrikam.com");
            Assert.Equal("avery@fabrikam.com", ConnectionRegistry.Find(r.Id).LastAccount);
        }
        ...
        public void The_account_hint_surfaces_on_the_query_side_of_the_connection_context()
        {
            ...
            var record = ConnectionRegistry.Remember("xmla", "powerbi://x/ctx", "Published", "Published", "contoso.com", "interactive", "megan@contoso.com");
            ...
            Assert.Equal("megan@contoso.com", context.Querying.Account);
        ...
        {
            ConnectionHistory.Append(new ConnectionHistoryEvent { Id = "abc", Kind = "connect", Account = "megan@contoso.com", Endpoint = "powerbi://x/h", Ok = true });
            ...
            Assert.Equal("megan@contoso.com", events[0].Account);
        }
        ...
        public void History_holds_no_secret()
        {
            ...
            ConnectionHistory.Append(new ConnectionHistoryEvent { Id = "s", Kind = "connect", Account = "megan@contoso.com", TenantId = "contoso.com", Endpoint = CredString("powerbi://x/s", null), Ok = true });
        ...
        {
            var r = ConnectionRegistry.Remember("xmla", "powerbi://x/probe3", "DS", "DS", "contoso.com", "azcli", "megan@contoso.com");
            ...
            try
            {
                ...
                Assert.Equal("megan@contoso.com", p.PreviousAccount); // surfaced ONLY as "last opened as" provenance
            }
            finally { EntraToken.SavedAccountForTests = null; }
        }
        ...
        {
            var r = ConnectionRegistry.Remember("xmla", "powerbi://x/mcp", "DS", "DS", "contoso.com", "interactive", "megan@contoso.com");
            ConnectionHistory.Append(new ConnectionHistoryEvent { Id = r.Id, Kind = "open", Account = "megan@contoso.com", Endpoint = r.Endpoint, Ok = true });
            EntraToken.SavedAccountForTests = (mode, tenant) => "megan@contoso.com";
            try
            {
                var sessions = new SessionManager();
                using var engine = new LocalEngine(sessions);
                var probes = await McpTools.ProbeConnectionAccounts(engine);
                Assert.Contains(probes, p => p.Id == r.Id && p.Account == "megan@contoso.com");
